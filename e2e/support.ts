...
export async function selectPaper(page: Page, paper: PaperFormat): Promise<void> {
  const target = PAPERS.formats.indexOf(paper);
  // 既に目的の書式なら、結線を毎回踏むために隣を経由してから戻る
  // (書式が 1 つだけなら送り先が無いので、そのまま検証へ進む)
  if (PAPERS.formats.length > 1 && (await paperIndex(page)) === target) {
    await stepPaper(page, target === 0 ? 1 : -1);
  }
  for (
    let guard = 0;
    guard < PAPERS.formats.length && (await paperIndex(page)) !== target;
    guard++
  ) {
...
async function paperIndex(page: Page): Promise<number> {
  ...
  return PAPERS.formats.findIndex((paper) => label.includes(paper.label));
}
