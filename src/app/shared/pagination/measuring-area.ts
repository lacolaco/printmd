...
export class MeasuringArea {
  ...
  measure(doc: RenderedDocument, ranges: readonly SegmentRange[]): Pagination {
    ...
    return this.collect(ranges, clones);
  }

  /** 計測中に何が起きても領域は取り除く */
  private collect(ranges: readonly SegmentRange[], clones: readonly HTMLElement[]): Pagination {
    try {
      return this.buildPagination(ranges, clones);
    } finally {
      this.host.remove();
    }
  }
