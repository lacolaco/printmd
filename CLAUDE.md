...
- switch は使わない: 原則禁止。使う場合は default を持たず、全 case が return し、網羅性が型で保証されている形に限る。lint ルール `printmd/no-switch` (default 禁止・全 case return) と `@typescript-eslint/switch-exhaustiveness-check` (型ベースの網羅性検査。type-aware lint) が強制し、`noImplicitReturns` が背後で漏れを拾う。適用対象は 5 行ルールと同じ。
