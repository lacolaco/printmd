...
describe('Preview', () => {
  ...
  it('原稿がなければシートを作らずページ数は 0', async () => {
    ...
    expect(TestBed.inject(Documents).pageCount()).toBe(0);
  });
  ...
  it('原稿があればマスターを複製したシートを作る (jsdom はレイアウトを持たないため 1 枚)', async () => {
    ...
    expect(TestBed.inject(Documents).pageCount()).toBe(1);
  });
  ...
  it('変換中はプレビュー面に進行表示を出し、完了したら消す', async () => {
    ...
    const el = fixture.nativeElement as HTMLElement;
    expect(TestBed.inject(Documents).rendering()).toBe(true);
    expect(el.querySelector('.app-rendering-indicator')).not.toBeNull();

    await fixture.whenStable();
    fixture.detectChanges();
    expect(TestBed.inject(Documents).rendering()).toBe(false);
