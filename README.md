...
ブラウザのフラグメンテーションエンジンは、印刷 (`@page`) だけでなく CSS 多段組でも同じコードで内容を「収まる単位」に割る。そこで画面では本文を `column-width: 178mm; height: 265mm` (A4 の版面) の多段組コンテナに流し込む。段の寸法が印刷の版面と一致するため、段 i の中身 = 印刷時のページ i の中身になる。
