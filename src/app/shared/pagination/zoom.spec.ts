...
import { ZOOMS, Zoom, fittingLevel } from './zoom';

/** A4 より大きい紙。カタログに何が並んでいても成り立たせるためこの場で組む */
const LARGE = new PaperFormat('大', { width: mm(297), height: mm(420), margin: mm(20) });

describe('fittingLevel', () => {
  it('広い画面では 100% (実寸) を上限とする', () => {
    expect(fittingLevel(2000, true, A4)).toBe(1);
  });

  it('A4 実寸が収まらない幅では収まる最大の段へ縮小する', () => {
    // 1100 - 360 - 48 = 692px < A4 794px → 75% (596px) が収まる最大
    expect(fittingLevel(1100, true, A4)).toBe(0.75);
  });

  it('スマートフォン幅では 50% まで下げる', () => {
    expect(fittingLevel(390, false, A4)).toBe(0.5);
  });

  it('どれも収まらない極端な幅でも最小段で止まる', () => {
    expect(fittingLevel(200, false, A4)).toBe(0.5);
  });
});

describe('紙の大きさと段', () => {
  it('同じ幅でも紙が大きい書式ではより小さい段を選ぶ', () => {
    expect(fittingLevel(1000, false, A4)).toBe(1);
    expect(fittingLevel(1000, false, LARGE)).toBe(0.75);
  });
});
