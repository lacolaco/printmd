...
/** 表示倍率。100% = 紙の実寸。現在値の保有と、切り替えと可否と表示文言を担う */
@Service()
export class Zoom {
  private readonly paper = inject(Paper);

  /** 現在の倍率 (1 = 紙の実寸)。書式が変われば収まる倍率へ戻す */
  ...
  /** 表示用の百分率文言 */
  readonly label = computed(() => ZOOMS.nameOf(this.value()));

  /** 倍率を delta ぶん動かす (両端で止まる) */
  ...
  isSteppable(delta: -1 | 1): boolean {
    return ZOOMS.isSteppable(this.value(), delta);
  }
}
